    ...
    with open(logpath, 'at') as flog:
        ...
        with open(successpath, 'wt') as fsuccess, open(failpath, 'wt') as ffail:
            with futures.ThreadPoolExecutor(max_workers= 8) as executor:
                future_to_image = {executor.submit(download_image, image, temproot, imageroot, flog=flog): image for image in images}
                for future in futures.as_completed(future_to_image):
                    ...
                    try:
                        ...
                        logexc('Unexpected exception for image {!r}.'.format(image), e, flog=flog)
